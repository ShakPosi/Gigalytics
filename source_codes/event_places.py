...
class EventPlaces():
    ...
    def __extract(self):
        return self._db.extract_table_to_pandas("musicbrainz.l_event_place", columns="id, entity0, entity1")
    
    def __transform(self, df):
        df["event_id"] = df["entity0"]
        df["place_id"] = df["entity1"]
        df = df.drop('entity0', 1)
        df = df.drop('entity1', 1)
        return df.sort_values(by=['id'], ascending=True)

    def __load(self, transformed_df):
        self._dw.load_df_into_dwh(transformed_df, "dim.event_places", "public")
